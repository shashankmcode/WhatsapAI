...
async function startHisoka() {
  ...
  const client = yesbhautikaiConnect({
    logger: pino({ level: "silent" }),
    printQRInTerminal: true,
    browser: ["Whatsapp-Ai-Bot - Yesbhautik", "Uni-explorer", "1.9"],
    auth: state,
  });
  ...
  client.ev.on("messages.upsert", async (chatUpdate) => {
    ...
    try {
      ...
      require("./yesbhautikai")(client, m, chatUpdate, store);
    } catch (err) {
      console.log(err);
    }
  });
